...
func getMonthKeyboard() tgbotapi.InlineKeyboardMarkup {
	keyboard := tgbotapi.NewInlineKeyboardMarkup(
		tgbotapi.NewInlineKeyboardRow(
			...
			tgbotapi.NewInlineKeyboardButtonData(time.Now().Add(time.Hour*(-31*24)).Month().String(), "month:"+strconv.Itoa(int(time.Now().Month())-1)),
			// tgbotapi.NewInlineKeyboardButtonData(time.Now().Add(time.Hour*(-31*48)).Month().String(), "/month "+strconv.Itoa(int(time.Now().Month())-2)),
		),
	)
	return keyboard
}
